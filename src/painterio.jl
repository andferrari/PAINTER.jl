...
function painterfitsexport(savepath::ASCIIString,PDATA::PAINTER_Data, OIDATA::PAINTER_Input; forceWvlExt=false)
	...
	# EX:
	#  painterfitsexport("/home/mvannier/.julia/v0.3/my_Painter/my_file.fits",PDATA,OIDATA)
	#  painterfitsexport("/home/mvannier/.julia/v0.3/my_Painter/my_file.fits",PDATA,OIDATA, forceWvlExt=true)
	...
	w=OIDATA.wvl
	write_wvl_ext= !all( ( diff(w) .< mean(diff(w)*(1+tol)) ) & ( diff(w) .> mean(diff(w)* (1-tol) )) ) | (forceWvlExt)
	...
	if (write_wvl_ext)
	  ...
	  fits_create_binary_tbl(f.fitsfile, size(w)[1], coldefs, "WAVELENGTHS")
